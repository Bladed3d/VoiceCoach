...
practice of using sub agents and that has
...
sub agent system. So firstly we got to
...
sub agent feature everything will be done
...
project with shadcn and then I will do
...
sub agents so that no it has this two sub
...
it trigger this shadcn agent. If I do
...
new subagent structure is so good
...
can ask it to connect to the vercel SDK.

So I ask it to let's do the vercel SDK

integration and make sure consult the

sub agent. So this will trigger the

vercel AI SDK implementation planner
...
for the vercel SDK and also update
...
Manus documentation: https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus
